...
def test_pipeline(df, nlp_pipeline, pipeline_name = ''):
    ...
    print pipeline_name + ' - kfolds log losses'
    print str([str(round(x, 3)) for x in sorted(losses)])
    print pipeline_name + ' - mean log loss: '
    print round(pd.np.mean(losses), 3)
    #print('{pipeline_name} kfolds log losses: {str([str(round(x, 3)) for x in sorted(losses)])}')
    #print('{pipeline_name} mean log loss: {round(pd.np.mean(losses), 3)}')
'''
...
class UnigramPredictions(TransformerMixin):
    ...
    def add_unigram_predictions(self, text_series):
        ...
        # the author.
        unigram_predictions = pd.DataFrame(self.unigram_mnb.predict_proba(text_series),
...
class PartOfSpeechFeatures(TransformerMixin):
    ...
    def add_pos_features(self, df):
        ...
        for part_of_speech in ['NOUN', 'VERB', 'ADJ', 'ADV']:
            df['{part_of_speech.lower()}iness'] = df['pos_counts'].apply(
                lambda x: self.part_of_speechiness(x, part_of_speech))
            df['{part_of_speech.lower()}iness'] /= df['sentence_length']
